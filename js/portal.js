...
document.getElementById('home-courses').innerHTML = SD.courses
  .filter(c => c.pct > 0)
  .slice(0, 3)
  .map(c => SD.makeCourseCard(c, `window.location.href='curso.html?id=${c.id}'`))
  .join('');
  ...
  document.getElementById('bib-list').innerHTML = allLessons.map(l => `
    <div class="lesson-item${l.done ? ' lesson-done' : ''}" style="margin-bottom:6px">
      ...
      ${l.done
        ...
        : '<button class="play-btn" onclick="window.location.href=\'ver-leccion.html\'"><i class="ti ti-player-play"></i>Ver</button>'}
    </div>`).join('');
}
renderBib('todos');
